...
function Avatar({ color, initial, size = 36, online }: { color: string; initial: string; size?: number; online?: boolean }) {
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      <div
        className={`w-full h-full rounded-full bg-gradient-to-br ${color} flex items-center justify-center text-white font-bold`}
...
interface SOSBannerProps { onDismiss: () => void; isNight: boolean; }
function SOSBanner({ onDismiss, isNight }: SOSBannerProps) {
  ...
  return (
    <div className="mx-3 mb-3 animate-slide-up">
      <div
        className="relative flex items-start gap-3 px-4 py-3 rounded-2xl border-2 border-radar-red animate-sos-flash"
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className={`text-xs mt-0.5 ${isNight ? 'text-slate-300' : 'text-slate-600'}`}>
            Solicitud de ayuda activa en tu ruta. Verifica la situación.
          </p>
        </div>
        ...
        <div
          className="absolute bottom-0 left-0 h-0.5 bg-radar-red rounded-full transition-all duration-1000"
...
export default function Chats() {
  ...
  async function sendMessage(isSos = false) {
    const text = isSos ? '🆘 NECESITO AYUDA — EMERGENCIA EN RUTA' : input.trim();
    if (!text) return;
    setSending(true);
    ...
    if (tab === 'group') {
      ...
      try {
        ...
        await queueMessage({ type: 'group', senderId: profile.id, roomId: '00000000-0000-0000-0000-000000000001', content: text, isSos, createdAt: new Date().toISOString() });
      }
    } else if (activeConvo) {
  ...
  const goldBg    = isNight ? 'bg-radar-gold/15 border-radar-gold/30' : 'bg-amber-50 border-amber-200';

  // ── Conversation thread view ──
  if (activeConvo && tab === 'private') {
    const msgs = privateMessages;
    return (
      <div className={`flex flex-col h-screen ${bg} transition-colors duration-700`}>
        ...
        <div className={`flex items-center gap-3 px-4 pt-12 pb-3 border-b ${surface} ${border}`}>
          <button onClick={() => setActiveConvo(null)} className={`${textSec} hover:${textPri} mr-1`}>
            <X size={18} />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
          {msgs.map((m) => (
            <div key={m.id} className={`flex ${m.isSelf ? 'justify-end' : 'justify-start'} animate-fade-in`}>
              <div className={`max-w-[75%] ${m.isSelf ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                <div
                  className={`px-4 py-2.5 rounded-2xl text-sm leading-snug ${
                    m.isSelf
                      ? isNight
                        ? 'bg-radar-gold/20 text-white rounded-br-sm border border-radar-gold/25'
                        : 'bg-amber-100 text-amber-900 rounded-br-sm'
                      : isNight
                        ? 'bg-night-card text-white rounded-bl-sm border border-night-border'
                        : 'bg-day-surface text-day-text rounded-bl-sm border border-day-border'
                  }`}
                >
                  {m.content}
                </div>
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className={`flex-1 px-4 py-3 rounded-xl border text-sm outline-none transition-all ${
                isNight
                  ? 'bg-night-card border-night-border text-white placeholder-slate-500 focus:border-radar-gold/50'
            ...
            <button
              ...
              className={`p-3 rounded-xl transition-all ${
                input.trim()
                  ? isNight
                    ? 'bg-radar-gold text-night-base shadow-neon-gold'
    ...
    <div className={`flex flex-col min-h-screen ${bg} transition-colors duration-700`}>
        ...
        <div className={`flex rounded-xl border overflow-hidden ${border}`} style={{ background: isNight ? '#0d1c3f' : '#f0f4ff' }}>
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium transition-all ${
              tab === 'private'
                ? isNight ? 'bg-radar-gold/15 text-radar-gold' : 'bg-amber-100 text-amber-700'
                : textSec
            }`}
          >
          ...
          {MOCK_CONVOS.map((c) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-4 text-left transition-all ${
                isNight ? 'hover:bg-night-surface' : 'hover:bg-day-surface'
              }`}
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {c.unread > 0 && (
                    <span className="w-5 h-5 rounded-full bg-radar-gold flex items-center justify-center text-[10px] font-bold text-night-base">
                      {c.unread}
                    </span>
                  )}
                </div>
      ...
      {tab === 'group' && (
        <div className="flex flex-col flex-1">
          ...
          {/* SOS banner (pinned, 10s auto-dismiss) */}
            ...
            {groupMessages.map((m) => (
              <div key={m.id} className={`flex ${m.isSelf ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                <div className={`flex gap-2 max-w-[80%] ${m.isSelf ? 'flex-row-reverse' : 'flex-row'}`}>
                  {!m.isSelf && (
                    <Avatar color={m.senderColor} initial={m.senderInitial} size={28} />
                  )}
                  <div className={`flex flex-col gap-1 ${m.isSelf ? 'items-end' : 'items-start'}`}>
                    {!m.isSelf && (
                      <span className={`text-[10px] font-medium ${gold}`}>@{m.senderName}</span>
                    )}
                    <div
                      className={`px-3 py-2 rounded-xl text-sm leading-snug ${
                        m.isSos
                          ? 'bg-radar-red/20 text-radar-red border border-radar-red/40 font-semibold'
                          : m.isSelf
                            ? isNight
                              ? 'bg-radar-gold/20 text-white border border-radar-gold/25'
                              : 'bg-amber-100 text-amber-900'
                            : isNight
                              ? `${card} text-white border ${border}`
                              : 'bg-day-surface text-day-text border border-day-border'
                      }`}
                    >
          ...
          <div className={`px-4 py-3 border-t ${surface} ${border} pb-28`}>
            ...
            <button
              ...
              style={{
                ...
                boxShadow: '0 0 20px 4px rgba(255,59,92,0.4)',
                color: 'white',
              }}
            >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`p-3 rounded-xl transition-all ${
                  input.trim()
                    ? isNight ? 'bg-radar-gold text-night-base shadow-neon-gold' : 'bg-amber-500 text-white'
                    : isNight ? 'bg-night-card text-night-muted' : 'bg-day-surface text-day-muted'
                }`}
              >
                <Send size={16} />
